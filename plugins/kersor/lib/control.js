import { a as hostCanonicalAuthorToolArguments, i as canonicalAuthorSaveCommand, n as canonicalAuthorCommandKind, r as canonicalAuthorHandoffSealCommand, t as hostNormalizableSetupArguments } from "./setup-tool-arguments-CGxY4VvS.js";
...
function canonicalAuthorCommands(authority) {
	return {
...
async function durableDispatchTransformationCallIds(agent, runDir) {
	const matches = [];
	for (const event of agent.session.events) {
		if (event.type !== "tool/call" || event.data.name !== "bash") continue;
		let argumentsValue;
		...
		const command = bashCommand(argumentsValue);
		if (command !== void 0 && await isRuntimeControlsCommand(command, runDir)) matches.push(event.data.callId);
	}
	return matches;
}
...
async function hostSelectedWorkflowName(runDir) {
	...
	if (selected === void 0 || selected.name === "STALLED") throw new Error(`${selectionName} must contain one non-STALLED selected_workflow`);
...
function baselineExecution(value, expectedKind, expectedCommand, sessionStarted, previousFinished) {
	const execution = record(value);
	if (execution === void 0 || !hasExactKeys(execution, BASELINE_EXECUTION_KEYS) || execution.kind !== expectedKind || execution.command !== expectedCommand || typeof execution.shell !== "string" || !isAbsolute(execution.shell) || execution.timed_out !== false || typeof execution.stdout !== "string" || typeof execution.stderr !== "string" || typeof execution.stdout_truncated !== "boolean" || typeof execution.stderr_truncated !== "boolean" || !Number.isSafeInteger(execution.exit_code) || expectedKind === "correctness" && execution.exit_code !== 0) throw new Error(`baseline ${expectedKind} execution evidence is invalid`);
...
async function validateBaselineWitness(authority) {
	...
	const correctness = baselineExecution(witness.executions[0], "correctness", authority.commands.correctness, sessionStarted, sessionStarted);
	const benchmark = baselineExecution(witness.executions[1], "benchmark", authority.commands.benchmark, sessionStarted, correctness.finished);
...
async function validateDispatchCustody(agent, runDir, envelope) {
	...
	if (transformation === void 0 || !hasExactKeys(transformation, TRANSFORMATION_RECEIPT_KEYS) || transformation.schema_version !== 1 || transformation.contract !== "dsh_dispatch_args_transformation_v1" || transformation.authority !== "dsh_host" || transformation.transformer !== "inject-runtime-controls" || transformation.session_dir !== sessionDir || transformation.run_dir !== runDir || transformation.round !== round || transformation.workflow_name !== workflowName || transformation.controller_session_id !== agent.id || !nonWhitespaceToken(transformation.controller_session_id) || !nonWhitespaceToken(transformation.transformation_call_id) || producerReceiptBinding?.sha256 !== producerHash || input === void 0 || !hasExactKeys(input, ["dispatch_args", "dispatch_args_provenance"]) || output === void 0 || !hasExactKeys(output, ["dispatch_args", "dispatch_args_provenance"]) || authorized === void 0 || !hasExactKeys(authorized, ["dispatch_args", "dispatch_args_provenance"])) throw new Error("dispatch runtime-control transformation receipt identity is invalid");
	...
	if (transformationCalls.length !== 1 || transformationCalls[0] !== transformation.transformation_call_id) throw new Error("dispatch transformation receipt lacks its exact durable canonical Bash call");
	...
	const transformationCallIndex = agent.session.events.findIndex((event) => event.type === "tool/call" && event.data.name === "bash" && event.data.callId === transformation.transformation_call_id);
	if (producerCallIndex < 0 || producerEventIndex <= producerCallIndex || transformationCallIndex <= producerEventIndex || transformationEventIndex <= transformationCallIndex) throw new Error("dispatch transformation durable event does not follow its producer event");
...
async function shellInvokesAuthorGate(executableWord, executable, operands, cwd, authority) {
	...
	if (![
		...
		"source",
		"zsh"
	].includes(executable)) return false;
	...
	const inspect = async (source, inheritedCwd, inheritedVariables, depth) => {
		...
		for (const words of shellCommands(tokens)) {
			...
			for (const word of invocationWords.slice(index + 1)) {
				...
				if (expanded === void 0) {
					if (SHELL_PATH_COMMANDS.has(initialExecutable) || SHELL_EXECUTION_WRAPPERS.has(initialExecutable)) return true;
					continue;
				}
...
function durableAuthorCommandCallIds(controller, command) {
	const calls = [];
	for (const event of controller.session.events) {
		if (event.type !== "tool/call" || event.data.name !== "bash") continue;
		try {
			if (hostCanonicalAuthorToolArguments(JSON.parse(event.data.arguments), command)) calls.push(event.data.callId);
		} catch {}
	}
	return calls;
}
async function authorHandoffBindings(authority) {
	const staging = await realpath(authority.stagingDir);
	if (staging !== authority.stagingDir) throw new Error("author staging directory contains a symlink or path alias");
	const entries = await readdir(staging, { withFileTypes: true });
	if (!isDeepStrictEqual(entries.map((entry) => entry.name).sort(), [...AUTHOR_STAGING_FILES].sort()) || entries.some((entry) => !entry.isFile() || entry.isSymbolicLink())) throw new Error("author staging must contain exactly the three direct author files");
	...
	let handoffValue;
	try {
		handoffValue = JSON.parse(handoffBytes.toString("utf8"));
	} catch {
	...
	if (handoff === void 0 || !hasExactKeys(handoff, [
		"schema_version",
		"staging",
		"files"
	]) || handoff.schema_version !== 1 || handoff.staging !== staging || sealedFiles === void 0 || !hasExactKeys(sealedFiles, AUTHOR_STAGING_FILES)) throw new Error("author handoff seal schema or staging identity is invalid");
	for (const name of AUTHOR_STAGING_FILES) if (sealedFiles[name] !== `sha256:${files[name].sha256}`) throw new Error(`author handoff changed after seal: ${name} hash mismatch`);
	return {
		handoff: {
...
async function beginAuthorHandoffSeal(authority, exec, command, hostGate) {
	const callId = exec.callId;
	const expected = canonicalAuthorHandoffSealCommand(authority.kersorPython, authority.sessionDir);
	if (command !== expected) return `author handoff sealing must use the exact Host-authorized command: ${expected}`;
	if (!hostCanonicalAuthorToolArguments(exec.arguments, expected)) return "author handoff sealing requires the exact foreground Host-owned Bash envelope";
	if (authorSealEvent(authority) !== void 0 || authorSaveEvents(authority).length !== 0 || await pathExists(authority.handoffPath)) return "author handoff sealing is exact-once and this Session is already sealed or consumed";
	const calls = durableAuthorCommandCallIds(authority.controller, expected);
	if (calls.length !== 1 || calls[0] !== callId) return "author handoff sealing is exact-once; a prior canonical attempt consumed this Session";
	hostGate.authorSealCalls.set(exec, {
		...authority,
		callId,
		command
	});
}
async function finishAuthorHandoffSeal(ctx, state) {
...
async function beginAuthorSaveAttempt(ctx, authority, callId, command, argumentsValue, hostGate) {
	const expected = canonicalAuthorSaveCommand(authority.kersorPython, authority.sessionDir);
	if (command !== expected) return `authored Proposal save must use the exact Host-authorized command: ${expected}`;
	if (!hostCanonicalAuthorToolArguments(argumentsValue, expected)) return "authored Proposal save requires the exact foreground Host-owned Bash envelope";
	const seal = authorSealEvent(authority);
	if (seal === void 0) return "authored Proposal save requires its durable Host author seal";
	if (hostGate.consumedAuthorSaves.has(authority.sessionDir) || authorSaveEvents(authority).length !== 0) return "authored Proposal save is exact-once; its canonical attempt is already consumed";
	const calls = durableAuthorCommandCallIds(authority.controller, expected);
	if (calls.length !== 1 || calls[0] !== callId) return "authored Proposal save is exact-once; a prior canonical attempt consumed this Session";
	const current = await authorHandoffBindings(authority);
	if (!isDeepStrictEqual(current.handoff, seal.data.handoff) || !isDeepStrictEqual(current.files, seal.data.files)) return "authored Proposal save input differs from its durable Host author seal";
	const event = {
		schema_version: 1,
...
async function canonicalRuntimeControlsCommand(runDir) {
	return `${frozenPythonPrefix(await frozenKersorPython())} bridge="\${DSH_HOME:-$HOME/.dsh}/.agent-presets/kersor/bin/kersor_bridge.py"; kersor_root="$("$KERSOR_PYTHON" "$bridge" root)"; "$KERSOR_PYTHON" "$kersor_root/scripts/inject-runtime-controls.py" ${shellQuote(runDir)}`;
}
...
async function isRuntimeControlsCommand(command, runDir) {
...
async function finishDispatchProducer(ctx, state, resultValue) {
...
async function dispatchTransformationContext(producer) {
...
async function beginDispatchTransformation(agent, callId, command, hostGate) {
	if (!command.includes("inject-runtime-controls.py")) return void 0;
	const produced = agent.session.events.filter((event) => event.type === "kersor/dispatch-args-produced");
	const matching = [];
	for (const event of produced) if (await isRuntimeControlsCommand(command, event.data.run_dir)) matching.push(event);
	if (matching.length !== 1) {
		const transformedRuns = new Set(agent.session.events.flatMap((event) => event.type === "kersor/dispatch-args-transformed" ? [event.data.run_dir] : []));
		const pending = produced.filter((event) => !transformedRuns.has(event.data.run_dir));
		const producer = pending.length === 1 ? pending[0]?.data : void 0;
	...
	if (matchingProducer === void 0) return "runtime-control injection lost its producer event";
	const producer = matchingProducer.data;
	if (!await isRuntimeControlsCommand(command, producer.run_dir)) return `runtime-control injection command is not the exact single deterministic injector invocation. Required: ${await canonicalRuntimeControlsCommand(producer.run_dir)}`;
	const priorCalls = await durableDispatchTransformationCallIds(agent, producer.run_dir);
	if (priorCalls.length !== 1 || priorCalls[0] !== callId) return "runtime-control injection is exact-once for this dispatch run";
	...
	if (!isDeepStrictEqual(JSON.parse(receiptBytes.toString("utf8")), producer)) return "producer receipt differs from the durable Host producer event";
	...
	if (!isDeepStrictEqual(currentArgs, producer.dispatch_args) || !isDeepStrictEqual(currentProvenance, producer.dispatch_args_provenance)) return "runtime-control injection input differs from the Host producer bytes";
	if (await pathExists(join(producer.run_dir, DISPATCH_TRANSFORMATION_RECEIPT))) return "dispatch transformation receipt already exists";
	hostGate.transformationCalls.set(callId, {
		...
		beforeArgs: await readBoundedJsonObject(producer.dispatch_args.path, MAX_DSH_DISPATCH_ARGS_BYTES, "dispatch-args.json"),
		beforeProvenance: await readBoundedJsonObject(producer.dispatch_args_provenance.path, MAX_DSH_DISPATCH_ARGS_BYTES, "dispatch-args-provenance.json")
	});
}
async function finishDispatchTransformation(ctx, state) {
	const afterArgs = await readBoundedJsonObject(state.producer.dispatch_args.path, MAX_DSH_DISPATCH_ARGS_BYTES, "dispatch-args.json");
	const afterProvenance = await readBoundedJsonObject(state.producer.dispatch_args_provenance.path, MAX_DSH_DISPATCH_ARGS_BYTES, "dispatch-args-provenance.json");
	...
	const outputProvenance = await dispatchFileBinding(state.producer.dispatch_args_provenance.path, "dispatch-args-provenance.json");
	const changed = !isDeepStrictEqual(outputArgs, state.producer.dispatch_args) || !isDeepStrictEqual(outputProvenance, state.producer.dispatch_args_provenance);
	const receipt = {
		schema_version: 1,
		...
		producer_receipt: {
			path: state.producerReceiptPath,
			sha256: state.producerReceiptSha256
		},
		input: {
...
async function beginBaselineWitness(ctx, agent, callId, command, launch, binding, hostGate) {
	...
	if (spec === void 0) return "baseline init/record/verify must use one exact canonical Host-authorized command";
...
function apply(ctx) {
	const workflowHostGate = {
		...
		transformationCalls: /* @__PURE__ */ new Map(),
		candidateSealCalls: /* @__PURE__ */ new Map(),
		baselineCalls: /* @__PURE__ */ new Map(),
		authorSealCalls: /* @__PURE__ */ new WeakMap(),
		consumedAuthorSaves: /* @__PURE__ */ new Set(),
	...
	ctx.on("tools/pre-execute", async (exec, next) => {
		...
		if (experimentAncestry !== void 0 && (exec.name === "bash" || authorPathTools.has(exec.name))) try {
			...
			if (exec.name === "bash" && authorAuthority !== void 0) authorCommand = canonicalAuthorCommandKind(exec.arguments, canonicalAuthorCommands(authorAuthority));
		} catch (error) {
			return {
				kind: "deny",
		...
		if (owned !== void 0) {
			...
			if (exec.name === "bash") {
				...
				if (command !== void 0 && authorCommand === "seal") {
					if (authorAuthority === void 0) return {
						kind: "deny",
						reason: "author handoff sealing requires one Host-initialized authoring Session"
					};
					try {
						const reason = await beginAuthorHandoffSeal(authorAuthority, exec, command, workflowHostGate);
						if (reason !== void 0) return {
							kind: "deny",
							reason
						};
				...
				} else if (command !== void 0 && authorCommand === "save") {
					if (authorAuthority === void 0) return {
						kind: "deny",
						reason: "authored Proposal save requires one Host-initialized authoring Session"
					};
					try {
						const reason = await beginAuthorSaveAttempt(ctx, authorAuthority, exec.callId, command, exec.arguments, workflowHostGate);
						if (reason !== void 0) return {
							kind: "deny",
							reason
						};
				...
				else if (command !== void 0 && command.includes("inject-runtime-controls.py")) try {
					const reason = await beginDispatchTransformation(agent, exec.callId, command, workflowHostGate);
					if (reason !== void 0) return {
						kind: "deny",
						reason
					};
		...
		if (exec.name === "bash" && experimentDescendant) {
			const command = bashCommand(exec.arguments);
			if (command !== void 0) {
				if (owned === void 0 && authorCommand !== void 0) return {
					kind: "deny",
				...
				if (owned === void 0 && command.includes("inject-runtime-controls.py")) return {
					kind: "deny",
		...
		if (exec.name === "bash" && owned !== void 0 && authorAuthority !== void 0 && authorCommand === void 0 && !acceptedNonAuthorHostBashGate) {
			if (bashCommand(exec.arguments) !== void 0 && await bashTouchesAuthorStaging(exec.arguments, authorAuthority, agent)) return {
				kind: "deny",
	...
	ctx.on("tools/post-execute", async (exec, result, next) => {
		...
		if (producerState !== void 0) try {
			if (decision.kind !== "accept" || result.isError) return decision;
			const transformationContext = await finishDispatchProducer(ctx, producerState, result.value);
			return {
				...decision,
				additionalContexts: [...decision.additionalContexts ?? [], transformationContext]
			};
		} catch (error) {
			return {
				kind: "block",
		...
		if (transformationState !== void 0) try {
			if (decision.kind !== "accept" || result.isError) return decision;
