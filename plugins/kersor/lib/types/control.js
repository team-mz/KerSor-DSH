...
import { canonicalAuthorCommandKind, canonicalAuthorHandoffSealCommand, canonicalAuthorSaveCommand, hostCanonicalAuthorToolArguments, } from "./author-tool-commands.js";
...
async function validateDispatchCustody(agent, runDir, envelope) {
    ...
    const transformationCallIndex = agent.session.events.findIndex(event => event.type === 'tool/call' && event.data.name === 'bash'
...
async function bashTouchesAuthorStaging(argumentsValue, authority, agent, stagingAccess = true) {
    ...
    const inspect = async (source, inheritedCwd, inheritedVariables, depth) => {
        ...
        for (const words of shellCommands(tokens)) {
            ...
            if (!SHELL_PATH_COMMANDS.has(executable))
                continue;
            if (!stagingAccess)
                continue;
...
async function beginAuthorSaveAttempt(ctx, authority, callId, command, argumentsValue, hostGate) {
    ...
    if (seal === undefined)
        return 'authored Proposal save requires its durable Host author seal';
...
async function beginDispatchTransformation(agent, callId, command, hostGate) {
    if (!command.includes('inject-runtime-controls.py'))
        return undefined;
    const produced = agent.session.events
        .filter((event) => event.type === 'kersor/dispatch-args-produced');
    const matching = [];
    for (const event of produced) {
        if (await isRuntimeControlsCommand(command, event.data.run_dir))
            matching.push(event);
    }
    ...
    if (matchingProducer === undefined)
        return 'runtime-control injection lost its producer event';
    ...
    if (!isDeepStrictEqual(receipt, producer))
        return 'producer receipt differs from the durable Host producer event';
...
export function apply(ctx) {
    const workflowHostGate = {
        ...
        transformationCalls: new Map(),
        candidateSealCalls: new Map(),
        baselineCalls: new Map(),
        authorSealCalls: new WeakMap(),
        consumedAuthorSaves: new Set(),
        ...
        if (experimentAncestry !== undefined
            ...
            try {
                ...
                if (exec.name === 'bash' && authorAuthority !== undefined) {
                    authorCommand = canonicalAuthorCommandKind(exec.arguments, canonicalAuthorCommands(authorAuthority));
                }
            }
            catch (error) {
                return {
