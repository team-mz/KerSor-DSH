 ...
 */
export function hostNormalizableSetupArguments(
  argumentsValue: unknown,
  expectedCommand: string,
  expectedWorkspace: string,
): boolean {
  if (argumentsValue === null || typeof argumentsValue !== 'object'
    || Array.isArray(argumentsValue)) return false
  ...
  if (argumentsRecord.run_in_background !== undefined
    && typeof argumentsRecord.run_in_background !== 'boolean') return false
  const permissions = argumentsRecord.sandbox_permissions
  if (permissions !== undefined
    && (typeof permissions !== 'string' || !BASH_SANDBOX_PERMISSIONS.has(permissions))) return false
