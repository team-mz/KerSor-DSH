...
async function readTerminalEvent(file: string): Promise<{
  discovery?: Extract<KersorRunDiscovery, 'completed' | 'failed'>
  issue?: KersorDiagnosticIssue
}> {
...
async function scanRun(
  ...
  kind: KersorRunKind,
  round?: number,
): Promise<{ ref?: KersorRunRef; issues: KersorScannedRunIssue[] }> {
  ...
  if (summary.value === undefined) {
    ...
    if (terminal.discovery !== undefined) discovery = terminal.discovery
    if (terminal.issue !== undefined) issues.push({ runDir, issue: terminal.issue })
  }
...
async function isGeneralTaskRun(dir: string): Promise<{ accepted: boolean; issue?: KersorDiagnosticIssue }> {
  try {
    const entries = await readdir(dir, { withFileTypes: true })
    return {
...
async function scanSession(
  ...
  const appendRun = async (
    ...
    if (scanned.ref !== undefined) runs.push(scanned.ref)
  }
...
export async function scanRoots(
  ...
  for (const candidate of candidates.values()) {
    ...
    for (const session of sessions) {
      ...
      if (generalTask.issue !== undefined) recordRootIssue(observation, generalTask.issue)
      if (generalTask.accepted) {
        const scanned = await scanRun(
          session.name, sessionDir, sessionDir, candidate.root, 'general-task',
        )
