...
| 自主 Workflow / Mission（只读或单文件事务） | KerSor preset → Host `kersor_evolve` → `runtime=dsh` → DSH `spawn` child | 固定 `deepseek-official/deepseek-v4-flash` route receipt、完整 usage、Core transaction／Host gate、`result.json` |
