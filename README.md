...
| 自主 Workflow / Mission（只读或单文件事务） | KerSor preset → Host `kersor_evolve` → `runtime=dsh` → DSH `spawn` child | 固定 `deepseek-official/deepseek-v4-flash` route receipt、durable usage／terminal evidence、Core transaction／Host gate、`result.json` |
| 超出 DSH 单文件事务边界的自主 Workflow / Mission | KerSor preset → Host `kersor_evolve` → 外部 Codex／Claude-compatible worker | `result.json`、artifact receipts、独立 verifier |
...
DSH-native Mission 现在接受 `side_effect=none|read`，也接受由 Mission authority 准入、Core 活事务绑定的单文件 `side_effect=write` capability。planner 与只读 worker 仍只看见 `read/glob/grep`；只有 Execute worker 的事务激活才额外看见 `edit/write`，并且同步 guard 在每次执行处只允许声明 artifact 的规范相对路径或对应绝对路径。agent 可以在同一事务中多次编辑该文件，但不能写其他路径、路径别名、链接、`.git`／`.conformance`／`.kersor*`、冻结 Mission、runtime config 或任意 Session 控制树，也不能调用 Bash、subagent、Workflow 或递归 KerSor。若 capability 声明 `candidate_verifier`，完整 candidate gate、rollback 策略与 evaluator request 都必须和冻结 Mission 逐字段匹配；只读 `command-v1` evaluator 仍由 Core 在事务快照存活时执行，通过才提交，失败、断连、取消或非完成输出均由 Core 回滚。Host 与 bridge 继续用同一合同 SHA 和 runtime 绑定一次读取，core 只能通过 owner-only AF_UNIX RPC 请求激活；每个 child 固定为 `deepseek-official/deepseek-v4-flash`。Host 按 `(turn, step)` 折叠 durable `assistant/chunk(type=usage)` 与 `assistant/message`，同一步以后出现的样本覆盖早期样本，并在检查 requested structured output 之前先处理 durable terminal error。只有固定 route 的唯一未计量 step 同时满足空输出、没有 content／message／usage、`finish` 与 `turn/end` 的 failure 都具有逐字精确的 raw machine code `QUOTA`（不修剪空白、不折叠大小写）和 HTTP 429 时，失败收据才可声明 `usage_observed=false`、`usage_complete=true` 和全零 usage；其余未观察到或无法证明完整的 usage 一律保留 `usage_complete=false`，不能伪造为零成本。每个顶层 DSH session 仍只能调用一次 `kersor_evolve`，失败、取消或 resume 都必须换新 session。

上述 known-zero 特例还要求完整 child event log 的 `seq` 从 0 开始无间隙，并且 fresh child 恰好有一个 `turn=1, step=1`、按序闭合的 `turn/start → step/start → QUOTA finish → step/end → turn/end` 生命周期，且 `turn/end` 必须是最后一个事件。合法的 inbox、user prompt、title 与 request metadata 可以位于这些边界之间；重复 turn／step、retry、任意 `tool/*`、缺失边界、乱序或终态后事件、任意其他 assistant 内容，或 finish／turn-end failure 的规范 JSON 事实不一致，都会降级为普通失败与 incomplete usage。一般 usage 也只有在该 fresh lifecycle 有效、step 从 1 连续且逐一闭合、每个 step 都有权威 token meter 时才可声明完整；`blocked`、`aborted`、`interrupted` 分别投影为 `refusal`、`aborted`、`error`。
